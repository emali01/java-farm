...
import './interfaces/IJavaSwapLottery.sol';

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title JavaSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract JavaSwapLottery is ReentrancyGuard, IJavaSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInJava = 50 ether;
    uint256 public minPriceTicketInJava = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInJava;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] javaPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInJava;
        uint32 finalNumber;
    }
     ...
     * @param _javaTokenAddress: address of the JAVA token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _javaTokenAddress, address _randomGeneratorAddress) {
        javaToken = IERC20(_javaTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of JAVA to this contract
        uint256 amountJavaToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInJava,
            _ticketNumbers.length
        );

        // Transfer java tokens to this contract
        javaToken.safeTransferFrom(address(msg.sender), address(this), amountJavaToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInJava += amountJavaToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInJavaToTransfer
        uint256 rewardInJavaToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInJavaToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        javaToken.safeTransfer(msg.sender, rewardInJavaToTransfer);

        emit TicketsClaim(msg.sender, rewardInJavaToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in JAVA per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInJava) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in JAVA for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].javaPerBracket[j] =
                ...
                // A. No JAVA to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].javaPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInJava - amountToShareToWinners);

        // Transfer JAVA to treasury address
        javaToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in JAVA token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        javaToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInJava += _amount;
     ...
     * @param _priceTicketInJava: price of a ticket in JAVA
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInJava >= minPriceTicketInJava) && (_priceTicketInJava <= maxPriceTicketInJava),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInJava: _priceTicketInJava,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            javaPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInJava: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(javaToken), "Cannot be JAVA token");
    ...
    /**
     * @notice Set JAVA price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInJava: minimum price of a ticket in JAVA
     * @param _maxPriceTicketInJava: maximum price of a ticket in JAVA
     */
    function setMinAndMaxTicketPriceInJava(uint256 _minPriceTicketInJava, uint256 _maxPriceTicketInJava)
        external
        onlyOwner
    {
        require(_minPriceTicketInJava <= _maxPriceTicketInJava, "minPrice must be < maxPrice");

        minPriceTicketInJava = _minPriceTicketInJava;
        maxPriceTicketInJava = _maxPriceTicketInJava;
    }
     ...
     * @param _priceTicket price of a ticket (in JAVA)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].javaPerBracket[_bracket];
        } else {
            return 0;
        }
    }
